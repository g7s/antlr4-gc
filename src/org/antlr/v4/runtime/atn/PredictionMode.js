...
PredictionMode.hasNonConflictingAltSet = function (altsets) {
    return find(altsets, alts => alts.cardinality() === 1) !== null;
};
...
PredictionMode.allSubsetsEqual = function (altsets) {
    var first = altsets[0];
    for(var i = 1; i < altsets.length; i++) {
        if (!altsets[i].equals(first)) return false;
    }
    return true;
};
...
PredictionMode.getUniqueAlt = function (altsets) {
    ...
    if (all.cardinality() === 1) return all.ntz();
